...
class Conv4(nn.Module):
    def __init__(self,img_channels=3,hidden_size=100,K=2,layers=1,latent_size=1024):
        super().__init__()
        self.latent_size = 512
        self.feature = models.resnet18()
        self.feature = torch.nn.Sequential(*(list(self.feature.children())[:-1]))
        self.auto_regressive = nn.GRU(self.latent_size,hidden_size,layers)
...
class LinClassifier(pl.LightningModule):
    ...
    def configure_optimizers(self):
        opt = torch.optim.Adam(self.parameters(),lr=5e-2)
        return opt
